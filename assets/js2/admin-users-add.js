$(document).ready(function() {
    ...
    var boxs = $.editboxInit({title: '学员信息',
        ...
        field: [{type: 'hidden', name: 'id'},
            {type: 'hidden', name: 'totalTime', value: '1440'},
            ...
            {type: 'text', name: 'ton', label: '从业资格证*：', validate: 'length', validateParam: '18', validateMsg: '从业资格证格式有误'},
            {type: 'text', name: 'birthday', label: '出身年月*：', validate: 'required', validateMsg: '出身年月有误', click: function() {
                    WdatePicker({dateFmt: 'yyyyMM', isShowWeek: true, minDate: '{%y-60}-%M-%d'});
                }},
            {type: 'select', name: 'gender', label: '性别：', options: gender},
            {type: 'text', name: 'signupTime', label: '报名时间*：', validate: 'required', editDisabled:false, validateMsg: '报名时间不能为空', click: function() {
                    WdatePicker({dateFmt: 'yyyy-MM-dd', isShowWeek: true});
                }},
            {type: 'text', name: 'code', label: '<b>激活码*</b>：', validate: 'required', editDisabled:false, validateMsg: '激活码不能为空', change: function() {
...
//            return subData;
//        },
        validating: function($ipt, name, val) {
            if (name == 'birthday' && val.length == 6) {
                var y = parseInt(val.substr(0, 4)) + 60;
                var m = val.substr(4, 2);
                var day = new Date(y + '/' + m + '/01');
                return day >= new Date();
            }
            return true;
        },
        onAddShow: function($box) {
            $box.editboxGetField('studyTypes').val(2);
        },
        onSubmited: function() {
    ...
    $('#copyUser').live('click', function () {

        $.post('action-admin-user-get', {id: sltId}, function(ret) {
            if (ret.resultCode === '0' || ret.resultCode === 0) {
                var initData = ret.obj;
                initData.idCard=initData.idCard.substr(0,18);
                initData.code='';
                initData.signupTime=today;
                initData.totalTime='1440';
                boxs.add.editboxShow({initData: initData});
                boxs.add.editboxGetField('avatar_').attr('src', 'data:image/jpeg;base64,' + initData.avatar);
            } else {
    ...
    $('#result').table({url: 'action-admin-user-page',
        ...
        toolbarDel: false,
        editbox: boxs,
        toolbarOther: '<a id="copyUser" class="btn btn-small disabled" ><i class="icon-btnadd" style="margin-top: 1px;"></i> 复制</a><a class="btn btn-small" href="admin-users"><i class="icon-btnequery" style="margin-top: 1px;"></i> 综合查询</a><a id="fromCVR" class="btn btn-small"><i class="icon-btnadd" style="margin-top: 1px;"></i> 报名登记</a>',
...
//            $('[name="add"]', $tab.parent()).addClass('ctrlHide');
//        }
    });

//    $('[name="filter.idCard"]').keypress(function(e) {
//        var theEvent = window.event || e;
//        var code = theEvent.keyCode || theEvent.which;
//        if (code == 43) {
//            $('.ctrlHide').removeClass('ctrlHide').removeClass('hidden');
//        } else if (code == 45) {
//            $('.ctrlHide2').removeClass('ctrlHide2').removeClass('hidden');
//        }
//    });
});