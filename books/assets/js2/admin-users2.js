...
//        },
    ...
    $('[name="order"]').live('click', function() {
        if ($(this).hasClass('disabled')) {
            return;
        }
        ...
        $.post('action-admin-reserve-order', {id: resId, userId: vals}, function(ret) {
            var $msg = $('#result [name="toolBar"] [name="msg"]');
            if (ret.resultCode === '0' || ret.resultCode === 0) {
...
//        toolbarDel: false,
        ...
        toolbarOther: '<span style="float:left;padding:2px 1px 0 3px">班级：<input type="text" class="input-small" id="cname"></span><a class="btn btn-small disabled" name="class"> 设置班级</a><a class="btn btn-small disabled" name="complete"> 现场学业</a><span style="float:left;padding:2px 1px 0 3px">预约：<input type="text" class="input-medium" name="reserve"></span><a class="btn btn-small disabled" name="order"> 预约</a>',
        field: [
